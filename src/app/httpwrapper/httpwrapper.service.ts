...
export class HttpWrapperService {
  ...
  protected defaultErrorInterceptor(resp: Response): any {
    ...
    if (typeof resp.json === 'function') {
      try {
        ...
        data.StackTrace = e.stack;
        data.ExceptionType = "InvalidJSONinDefaultErrorInterceptor";
      }
    } else {
  ...
  get<T>(url: string, options?: RequestOptionsArgs): Observable<T> {
    return this.http.get(this.generateUrl(url), this.generateOptions(options)).pipe(
  ...
  post<T>(url: string, data: Object, options?: RequestOptionsArgs): Observable<T> {
    ...
    return this.http.post(this.generateUrl(url), newData, this.generateOptions(options)).pipe(map(this.responseHandler, this)
      , catchError(this.errorHandler.bind(this)));
  }
  ...
  put<T>(url: string, data: Object, options?: RequestOptionsArgs): Observable<T> {
    const newData = this.prepareData(data);
    return this.http.put(this.generateUrl(url), newData, this.generateOptions(options)).pipe(map(this.responseHandler, this), catchError(this.errorHandler.bind(this)));
  }
  ...
  patch<T>(url: string, data: Object, options?: RequestOptionsArgs): Observable<T> {
    const newData = this.prepareData(data);
    return this.http.patch(this.generateUrl(url), newData, this.generateOptions(options)).pipe(
      map(this.responseHandler, this)
      , catchError(this.errorHandler.bind(this)));
  }
  ...
  delete<T>(url: string, options?: RequestOptionsArgs): Observable<T> {
    return this.http.delete(this.generateUrl(url), this.generateOptions(options)).pipe(
      map(this.responseHandler, this), catchError(this.errorHandler.bind(this)));
  }
  ...
  protected errorHandler(error: Response): Observable<any> {
    return throwError(this.errorInterceptors.reduce((acc: any, interceptor: any) => interceptor(acc), error));
  }
  ...
  protected generateUrl(url: string): string {
    return url.match(urlPattern) ? url : this.baseUrl + url;
  }
